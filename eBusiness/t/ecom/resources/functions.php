...
$product = <<<DELIMITER
            ...
            <div class="col-md-3 col-sm-6 hero-feature">
                <div class="thumbnail">
                    <img src="../resources/{$product_image}" style="height:110px" alt="Image Not Found" onerror="this.onerror=null;this.src='https://place-hold.it/320x150/3ec1d5?text=This Is A Replacement Image By Mohit !';">
                    <div class="caption">
...
{
	$query = query("SELECT * FROM orders WHERE Status!='Order Complete'");
	confirm($query);
	$sl_no = 0;
	while($row = fetch_array($query))
	{	
		...
		$orders = <<<DELIMITER
		<tr>
		<td>{$sl_no}</td><a href="index.php?edit_products&id={$row['product_id']}">{$row['product_title']}</a>
		<td>{$row['order_id']}</td>
        <td>{$row['order_amount']}</td>
        <td>{$row['tran_id']}</td>
        <td>{$row['order_date']}</td>
        <td>{$row['Status']}</td>
        <td><a class='btn btn-update' href="index.php?delete_order_id={$row['order_id']}"><span class='glyphicon glyphicon-remove'></span></a></td>
        </tr>
DELIMITER;
		echo $orders;
	}
